...
- 开发时在关键流程、外部调用、状态迁移、失败重试和错误返回处补充必要日志，方便出问题时按时间线分析；日志应包含操作名、阶段、非敏感上下文和错误原因，不记录 token、refresh token、API key、密文、完整 auth JSON 等敏感数据。
...
- 未经用户明确说“发布”前，不要 push main、不要 push tag、不要创建 release；普通改动只做本地实现、必要测试和说明。改完并测试后，不要立即本地打包验证；只有用户明确说“打包验证”或“本地打包”时，才执行本地打包、覆盖安装或运行验证。
